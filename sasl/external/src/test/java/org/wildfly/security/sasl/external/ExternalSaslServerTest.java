...
//
//    @Test
//    public void testSkipCertVerificationProp() throws SaslException, URISyntaxException, RealmUnavailableException {
//        Path rootPath = Paths.get(getClass().getResource(File.separator).toURI())
//                .resolve("filesystem-realm");
//
//        FileSystemSecurityRealm securityRealm = new FileSystemSecurityRealm(rootPath, 3);
...
//
//        SaslServer server = builder.build().createMechanism("EXTERNAL");
////        server.get
//        assertNotNull("SaslServerFactory not registered", factory);
//// securityDomain has jdbc realm with identity,
//
//        ServerAuthenticationContext sac = securityDomain.createNewAuthenticationContext();
//
//        sac.setAuthenticationName("username");
//
//        SaslServer server2 = factory.createSaslServer(EXTERNAL, "test", "localhost", setProps("org.wildfly.sasl.skip-certificate-verification"), sac.createCallbackHandler());
//        assertNotNull(server);
//        assertEquals(ExternalSaslServer.class, server.getClass());
//    }
//
