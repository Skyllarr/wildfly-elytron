...
synopsis: The HashiCorp Vault feature pack is available for provisioning with WildFly 40 Beta.
author: dvilkola
---
...
Replace `${wildfly.version}` with the WildFly 40 Beta Galleon pack version and `${vault.pack.version}` with the latest `org.wildfly.security.vault:wildfly-vault-feature-pack` release on Maven Central:
...
**IMPORTANT NOTE:** The location of this feature pack will most likely change in the WildFly 41 release, Specifically, the `hahsicorp-vault` subsystem layer will likely be included in the wildfly-ee feature pack.
